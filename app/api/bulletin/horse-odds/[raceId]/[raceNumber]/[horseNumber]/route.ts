...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ raceId: string; raceNumber: string; horseNumber: string }> }
) {
  try {
    const { raceId, raceNumber, horseNumber } = await params
    const raceNumberInt = parseInt(raceNumber)
    const horseNumberInt = parseInt(horseNumber)

    if (!raceId || !raceNumberInt || !horseNumberInt) {
      return NextResponse.json(
        { error: 'Race ID, Race Number ve Horse Number gerekli' },
        { status: 400 }
      )
    }
    ...
    const targetRace = raceData.runs.find((run: any) => run.number === raceNumberInt)
    if (!targetRace) {
      return NextResponse.json(
        { error: `Race ${raceNumberInt} bulunamadı` },
        { status: 404 }
      )
    }

    // Belirtilen at numarasını bul
    const targetHorse = targetRace.horses?.find((horse: any) => horse.no === horseNumberInt)
    if (!targetHorse) {
      return NextResponse.json(
        { error: `Horse ${horseNumberInt} bulunamadı` },
        { status: 404 }
      )
    }
    ...
    const result = {
      ...
      race_number: raceNumberInt,
      horse_number: horseNumberInt,
      horse_name: targetHorse.name,
      odds: targetHorse.fixedOdd,
      jockey: targetHorse.jockeyName,
      trainer: targetHorse.coachName,
      weight: targetHorse.weight,
      age: targetHorse.age,
      sex: targetHorse.gender
    }
