...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ raceId: string; raceNumber: string }> }
) {
  try {
    const { raceId, raceNumber } = await params
    const raceNumberInt = parseInt(raceNumber)

    if (!raceId || !raceNumberInt) {
      return NextResponse.json(
        { error: 'Race ID ve Race Number gerekli' },
        { status: 400 }
      )
    }
    ...
    const horses = targetRace.horses?.map((horse: any) => ({
      horse_no: horse.no,
      horse_name: horse.name,
      odds: horse.fixedOdd,
      jockey: horse.jockeyName,
      trainer: horse.coachName,
      weight: horse.weight,
      age: horse.age,
      sex: horse.gender
    })) || []
    ...
    const result = {
      ...
      race_name: targetRace.info,
      race_time: targetRace.time,
      horses: horses,
